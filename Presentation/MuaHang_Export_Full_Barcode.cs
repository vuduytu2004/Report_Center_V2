...
{
    public partial class MuaHang_Export_Full_Barcode : Form
    {

        public MuaHang_Export_Full_Barcode()
        {
            InitializeComponent();
            Shown += new EventHandler(MuaHang_Export_Full_Barcode_Shown);
        ...
        DataTable table_nguon = new DataTable();
        ...
        private void Exit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                MuaHang_Export_Full_Barcode f3 = (MuaHang_Export_Full_Barcode)Application.OpenForms["MuaHang_Export_Full_Barcode"];
                f3.Close();
            }
        ...
        void MuaHang_Export_Full_Barcode_Shown(object sender, EventArgs e)
        {
        ...
        void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            /// -----------------------------test//----------------------------------------------------------------------------------
            string sql1 = "MuaHang_Export_Full_Barcode";

            //dataGridView_full.DataSource = cn.taobang1(sql);
            table_nguon = cn.taobang_NO_Para_All(sql1, 3);

            //table = cn.taobang1(sql);
            setDataSource(table_nguon);
        ...
        private void MuaHang_Export_Full_Barcode_Load(object sender, EventArgs e)
        {
...
LEFT join  DSMART12.dbo.RTPRICE AS CU WITH (NOLOCK) ON CU.SKU_ID = a.SKU_ID
        ...
        private void tk_Full_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridView_full.RowCount == 0 || Ma_NCC.TextLength == 0)
            { return; }
            ...
            DataTable dataTable11 = ((DataTable)dataGridView_full.DataSource);

            //dataTable.Rows.Count
            // Tạo DataView từ DataTable để áp dụng row filter
            DataView dataView = new DataView(dataTable11);

            // Xây dựng biểu thức điều kiện sử dụng toán tử OR giữa các cột
            string condition = string.Empty;

            foreach (DataColumn column in dataTable11.Columns)
            {
                ...
                condition += string.Format("[{0}] LIKE '%{1}%'", column.ColumnName, Ma_NCC.Text.Trim());
            }

            // Áp dụng row filter chỉ hiển thị các hàng thỏa mãn điều kiện
            dataView.RowFilter = condition;

            //DataView dataView = (DataView)dataGridView.DataSource;

            // Tạo một DataTable mới và sao chép cấu trúc từ DataView
            DataTable dataTable = dataView.ToTable();

            // Hiển thị DataTable trên DataGridView
            dataGridView_full.DataSource = dataTable;


            //// Gán DataView đã lọc lại cho nguồn dữ liệu của DataGridView
            //dataGridView_full.DataSource = dataView;


            //string sql1 = "MuaHang_Export_Full_Barcode";

            ////dataGridView_full.DataSource = cn.taobang1(sql);
            //dataGridView_full.DataSource = cn.taobang_NO_Para_All(sql1, 3);
            progressBar1.Visible = false;
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                if (table_nguon.Rows.Count > 0)
                {
                    dataGridView_full.DataSource = table_nguon;
                    return;
                }
                progressBar1.Visible = true;
                progressBar1.Style = ProgressBarStyle.Marquee;
        ...
        private void export_full_Click(object sender, EventArgs e)
        {
            ...
            string filePath = "D:\\Full_Barcode_" + DateTime.Today.Day + DateTime.Today.Month + DateTime.Today.Year + "_" + n + ".xlsx";
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            //DataSet dataSet = GetYourDataSet(); // Thay thế hàm này bằng cách lấy dữ liệu của bạn
            //DataSet dataSet = DataGridViewToDataSet.GetDataSetFromDataGridView(dataGridView_full);

            //string filePath = "d:/file.xlsx";
            //ExcelExporter.ExportToExcel(dataSet, filePath);
            progressBar1.Visible = true;
            ...
            //System.Diagnostics.Process.Start("D:\\Zone_Price_" + DateTime.Today.Day + DateTime.Today.Month + DateTime.Today.Year + "_" + n + ".xlsx");
            string filePath = "D:\\Full_Barcode_" + DateTime.Today.Day + DateTime.Today.Month + DateTime.Today.Year + "_" + n + ".xlsx";
            //System.Threading.Thread thread = new System.Threading.Thread(new System.Threading.ThreadStart(ExcelExporter.ExportDataGridViewToExcel(dataGridView_full, filePath)));
            System.Threading.Thread thread = new System.Threading.Thread(() =>
            {
                ExcelExporter.ExportDataGridViewToExcel(dataGridView_full, filePath);
            });
        ...
        {
            public static void ExportDataGridViewToExcel1(DataGridView dataGridView, string filePath)
            {
                if (dataGridView.Rows.Count == 0)
                {
                    // Xử lý khi DataGridView không có hàng dữ liệu
                    return;
                }
                ...
                foreach (DataGridViewRow row in dataGridView.Rows)
                {
                    ...
                    foreach (DataGridViewCell cell in row.Cells)
                    {
                        ...
                        else
                        {
                            ...
                            dataRow[cell.ColumnIndex] = DBNull.Value; // hoặc gán giá trị mặc định khác tùy vào yêu cầu
                        }
                    }
                ...
                {
                    ExcelWorksheet worksheet = package.Workbook.Worksheets.Add("Full_Barcode");
